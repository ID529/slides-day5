...
::: {.white-12-pt}
Dean Marengi \| Friday, January 17<sup>th</sup>, 2025
:::
...
-   Combine observations from two datasets, but treats observations as set elements
    -   i.e., treat rows in each table as individual items in a set
        ...
        -   Find rows in both tables ([intersection]{.blue-bold})
        -   Find rows in either table, but not both ([union]{.blue-bold})
        -   Find rows in one table, but not other ([difference]{.blue-bold})
...
```{r, echo=T, eval=F}
left_join(
  x, # dataset 1 (i.e., the dataset on the left side)
  y, # dataset 2 (i.e., the dataset on the right side)
  by = NULL,
  suffix = c(".x", ".y"),
  keep = FALSE
)
...
<!-- -   bind_rows() and bind_cols() slide -->
<!-- -   crosswalk for id's example (and image showing map from dataset 1 --> 2 --> 3) -->
<!-- -   one to many example  -->
<!-- -   merging 3+ tables example -->
