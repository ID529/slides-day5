...
- Also available in the code (but we won't go over it today)
  - the `sjPlot` package
...
- As we covered yesterday, we begin by doing some data cleaning steps.
...
comparison <- ggplot(lm_education_estimates, aes(x=category, y=estimate, color=model, shape=model)) +
